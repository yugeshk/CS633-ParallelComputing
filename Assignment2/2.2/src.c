...
int main(int argc, char *argv[]){
    ...
    int data_size[5] = {65536, 262144, 524288, 2097152, 4194304};
    ...
    if(my_rank == 0){
        ...
        for(int iter = 0; iter < world_size -1 ; iter++){
            ...
            if(my_rank % 2 == 0){
                MPI_Send(recv_buff + send_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, my_rank+1, 99, MPI_COMM_WORLD);
                MPI_Recv(recv_buff + recv_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, world_size-1, 99, MPI_COMM_WORLD, &status);

            }
            else{
                MPI_Recv(recv_buff + recv_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, my_rank -1, 99, MPI_COMM_WORLD, &status);
                MPI_Send(recv_buff + send_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, my_rank+1, 99, MPI_COMM_WORLD);
        ...
        for(int iter = 0; iter < world_size -1 ; iter++){
            ...
            if(my_rank % 2 == 0){
                ...
                MPI_Recv(recv_buff + recv_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, my_rank -1, 99, MPI_COMM_WORLD, &status);
                MPI_Send(recv_buff + send_ptr, data_size[i]/world_size, MPI_UNSIGNED_CHAR, (my_rank+1)%world_size, 99, MPI_COMM_WORLD);
