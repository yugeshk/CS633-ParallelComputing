...
void kmeans_process(data_struct *data_in, data_struct *clusters, double *newCentroids, double* SumOfDist)
{
	...
	for (int i = 0; i < clusters->secondary_dim; i++){
		cluster_size[i] = 0;
	}

	for (int i = 0; i < data_in->secondary_dim; i++){
		tmp_dist = 0;
		tmp_index = 0;
		min_dist = FLT_MAX;

		/*find nearest center*/
		for (int k = 0; k < clusters->secondary_dim; k++){
		...
		for (int j = 0; j < data_in->leading_dim; j++)
		{
...
void read_dataset_file(data_struct *data_in, int world_size, int my_rank, char *inp_file){
    ...
    for(int i=0;i<pts;i++){ //Read all points for all processes
        t_Index[i]=0;
...
int main(int argc, char **argv){ 
	...
	if(my_rank != world_size-1){
        for(int i = 0; i < p_data.secondary_dim * p_data.leading_dim; i++){ 
	...
	for (int i = 0; i < p_data.secondary_dim; i++){ 
